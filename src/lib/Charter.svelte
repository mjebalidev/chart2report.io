<script>
    import { Button, Dropdown, DropdownItem, Chevron, Checkbox , Label, P, Tabs, TabItem, Input, Alert, Progressbar } from 'flowbite-svelte'
  ...
  function renderXML(){
    ...
    finalXML = headerXML + kpiXML + boothVisitsOverTimeXML + boothVisitsXML + videoViewsXML + downloadsXML + boothChatsXML + videoChatsPerBoothXML + "</report>"
    display = true;
  }
...
<div class="main-frame">
  <Tabs>
    <TabItem open title="Informations" on:click={ProgressBar0}>
      <form>
        <div class="grid gap-4 mb-4 md:grid-cols-2">
          ...
          <div>
            ...
            <Input type="text" id="event-name" required bind:value={eventName}/>
          </div>
          <div>
          ...
          <div>
            ...
            <Input type="text" id="profile-1-title" bind:value={profile1title}/>
          </div>
          <div>
          ...
          <div>
            ...
            <Input type="text" id="profile-2-title" bind:value={profile2title}/>
          </div>
        </div>
      </form>
      <Button on:click={saveData} type="submit">Save</Button>
    </TabItem>
    <TabItem title="Charts" on:click={ProgressBar50}>
      <div>
        ...
        <div>
          ...
          <Button color="green" on:click={AddBoothVisitsOverTime}>Enable</Button>
          <Button color="red" on:click={RemoveBoothVisitsOverTime}>Disable</Button>
        </div>
        ...
        <div>
          ...
          <Button color="green" on:click={AddVideochatsPerBooth}>Enable</Button>
          <Button color="red" on:click={RemoveVideochatsPerBooth}>Disable</Button>
        </div>
      ...
      <Dropdown >
        <DropdownItem on:click={() => dropDownCounter = 1}>KPIs</DropdownItem>
        <DropdownItem on:click={() => dropDownCounter = 2}>Booth visits over time</DropdownItem>
        <DropdownItem on:click={() => dropDownCounter = 3}>Booth Visits</DropdownItem>
        <DropdownItem on:click={() => dropDownCounter = 4}>Video Views</DropdownItem>
        <DropdownItem on:click={() => dropDownCounter = 5}>Downloads</DropdownItem>
        <DropdownItem on:click={() => dropDownCounter = 6}>Booth Chats</DropdownItem>
        <DropdownItem on:click={() => dropDownCounter = 7}>Videochats per Booth</DropdownItem>
        <DropdownItem on:click={() => dropDownCounter = 8}>All Charts</DropdownItem>
      </Dropdown>
      </TabItem>
      <TabItem on:click={ProgressBar100} title="Result">
        ...
        <div>
          ...
          <P class="mb-3" weight="light" color="text-gray-500 dark:text-gray-400">{finalXML}</P> 
          <Button color="green" on:click={copyToClipboard}> Copy to clipboard </Button>
        </div>
        {/if}
    </TabItem>
  </Tabs>
  ...
  <div class="alert-saved">
    <Alert dismissable color="blue">Data have been correctly saved, please proceed to the next step.</Alert>
  </div>
  {/if}
  {#if alertAll}
  <div class="alerts">
  ...
  <div class="alerts">
    <Alert dismissable color="green">Booth visits over time chart has been selected!</Alert>
  </div>
  {/if}
  {#if alertBoothVisits}
  <div class="alerts">
    <Alert dismissable color="green">Booth visits chart has been selected!</Alert>
  </div>
  {/if}
  {#if alertVideoViews}
  <div class="alerts">
    <Alert dismissable color="green">Video views chart has been selected!</Alert>
  </div>
  {/if}
  {#if alertDownloads}
  <div class="alerts">
  ...
  {#if alertBoothChats}
  <div class="alerts">
    <Alert dismissable color="green">Booth chats chart has been selected!</Alert>
  </div>
  {/if}
  {#if alertVideochatsPerBooth}
  <div class="alerts">
    <Alert dismissable color="green">Videochats per booth chart has been selected!</Alert>
  </div>
  {/if}
  {#if alertDeactivationAll}
  <div class="alerts">
    <Alert dismissable color="red">All the charts have been deactivated!</Alert>
  </div>
  {/if}
  ...
  <div class="alerts">
    <Alert dismissable color="red">Booth visits over time chart has been deactivated!</Alert>
  </div>
  {/if}
  {#if alertDeactivationBoothVisits}
  <div class="alerts">
    <Alert dismissable color="red">Booth visits chart has been deactivated!</Alert>
  </div>
  {/if}
  {#if alertDeactivationVideoViews}
  <div class="alerts">
    <Alert dismissable color="red">Video views chart has been deactivated!</Alert>
  </div>
  {/if}
  {#if alertDeactivationDownloads}
  <div class="alerts">
    <Alert dismissable color="red">Downloads chart has been deactivated!</Alert>
  </div>
  {/if}
  {#if alertDeactivationBoothChats}
  <div class="alerts">
    <Alert dismissable color="red">Booth chats chart has been deactivated!</Alert>
  </div>
  {/if}
  {#if alertDeactivationVideochatsPerBooth}
  <div class="alerts">
    <Alert dismissable color="red">Videochats per booth chart has been deactivated!</Alert>
  </div>
  {/if}
</div>
